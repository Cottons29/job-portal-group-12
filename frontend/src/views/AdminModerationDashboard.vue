<template>
  <div class="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
    ...
    <aside class="w-64 bg-white shadow-sm flex flex-col justify-between flex-shrink-0 z-10">
      <div>
        ...
        <nav class="mt-2 px-4 flex flex-col gap-1">
          ...
          <a href="#" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-blue-50 text-blue-600 font-bold transition-colors">
            Pending Employers
          </a>
          <a href="#" class="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors font-medium">
            Verified SMEs
      ...
      <div class="p-4 px-4 flex flex-col gap-1 mb-4 border-t border-slate-100/50">
        <a href="#" class="flex items-center gap-3 px-4 py-2 rounded-lg text-slate-500 hover:bg-slate-50 transition-colors text-sm font-medium">
          Support
        </a>
        <a href="#" class="flex items-center gap-3 px-4 py-2 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors text-sm font-medium">
          Log out
        </a>
      </div>
    </aside>
    ...
    <main class="flex-1 overflow-y-auto p-8 lg:px-12 flex flex-col gap-8">
      ...
      <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div class="max-w-2xl">
          ...
          <p class="text-slate-500 text-sm leading-relaxed">
            Review and approve new employer registrations to grant access to the talent pool. Ensure all submitted Business Patents are valid.
          </p>
        </div>
        
        <!-- Search & Filter -->
        <div class="flex items-center gap-3 flex-shrink-0">
          <div class="relative bg-white rounded-lg shadow-sm flex items-center px-3 py-2 w-64 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
            <MagnifyingGlassIcon class="w-5 h-5 text-slate-400" />
            <input type="text" placeholder="Search employers..." class="w-full bg-transparent border-none focus:ring-0 text-sm px-2 text-slate-700 placeholder:text-slate-400 outline-none" />
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 flex flex-col justify-center">
          ...
          <span class="text-4xl font-display font-bold text-slate-800">{{ averageWaitTime }} <span class="text-lg text-slate-400 font-medium">days</span></span>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm flex flex-col">
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50/80">
              <tr v-for="employer in pendingEmployers" :key="employer.id" class="hover:bg-slate-50/50 transition-colors group">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-500 transition-colors">
                ...
                <td class="px-6 py-4">
                  <span v-if="employer.status === 'Pending Review'" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700">
                    Pending Review
                  </span>
                  <span v-else-if="employer.status === 'Action Required'" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700">
                    Action Required
                  </span>
                </td>
                <td class="px-6 py-4 text-right">
                  <div class="flex justify-end">
                    <button @click="openModal(employer)" class="w-10 h-10 rounded-full flex items-center justify-center text-blue-600 hover:bg-blue-50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-100">
        ...
        <div class="px-6 py-4 border-t border-slate-50 flex items-center justify-between text-sm text-slate-500">
          ...
          <div class="flex items-center gap-1">
            <button class="p-1 hover:text-blue-600 disabled:opacity-50"><ChevronLeftIcon class="w-5 h-5" /></button>
            <button class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 font-bold flex items-center justify-center">1</button>
            <button class="p-1 hover:text-blue-600 disabled:opacity-50"><ChevronRightIcon class="w-5 h-5" /></button>
          </div>
        </div>
      </div>
    ...
    <!-- PatentViewerModal -->
    <div v-if="selectedEmployer" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
      <div class="bg-white w-full max-w-3xl rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="flex items-center justify-between p-6 pb-4">
          ...
          <button @click="closeModal" class="text-slate-400 hover:text-slate-700 transition-colors p-2 rounded-full hover:bg-slate-100">
        ...
        <div class="p-6 bg-slate-50/50 flex-1 overflow-y-auto">
          ...
          <div class="w-full bg-white rounded-xl shadow-sm flex flex-col items-center justify-center border border-slate-100 aspect-[1/1.4] sm:aspect-[4/3]">
            <div class="flex flex-col items-center text-center p-8">
              ...
              <span class="text-xs font-mono text-slate-400 mt-6 bg-slate-50 px-3 py-1 rounded-md">{{ selectedEmployer.patentUrl }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 flex gap-4 bg-white border-t border-slate-50">
          <button @click="approveEmployer" class="flex-1 bg-green-500 hover:bg-green-600 text-white py-3.5 rounded-xl font-bold transition-colors shadow-sm flex items-center justify-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
            Approve Employer
          </button>
          <button @click="rejectEmployer" class="flex-1 bg-transparent border-2 border-red-100 text-red-600 hover:bg-red-50 py-3.5 rounded-xl font-bold transition-colors flex items-center justify-center gap-2">
  ...
  try {
    ...
    pendingEmployers.value = response.data.map(emp => {
      ...
      return {
        ...
        appliedDate: new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).format(applied),
