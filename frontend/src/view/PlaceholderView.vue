        ...
        <div v-else-if="activePage === 'settings'"
          ...
          <section v-else-if="activeSettingsSection === 'security'" class="mx-auto min-w-39/40 max-w-2xl">
            ...
            <div class="mt-3 space-y-2">
              <p v-if="passkeyMessage" class="rounded-2xl bg-[#8fd99b]/10 px-4 py-3 text-xs font-bold text-[#8fd99b]">{{ passkeyMessage }}</p>
              <p v-if="passkeyError" class="rounded-2xl bg-red-500/10 px-4 py-3 text-xs font-bold text-red-300">{{ passkeyError }}</p>
              <p v-if="passwordError" class="rounded-2xl bg-red-500/10 px-4 py-3 text-xs font-bold text-red-300">{{ passwordError }}</p>
              <p v-if="passwordMessage" class="rounded-2xl bg-[#8fd99b]/10 px-4 py-3 text-xs font-bold text-[#8fd99b]">{{ passwordMessage }}</p>
            </div>
          </section>

          <section v-else class="mx-auto min-w-39/40 max-w-2xl">
            <h2 class="mb-4 text-center font-display text-xl font-black tracking-[-0.04em] text-on-surface sm:text-2xl">
              Data & privacy
            </h2>
            <div class="rounded-[1.25rem] bg-surface-container-low p-5 text-sm font-bold text-on-surface-variant ring-1 ring-white/5">
...
const settingsMenuItems = computed(() => [
  ...
  {label: 'Security & sign-in', section: 'security', icon: LockClosedIcon, bg: 'bg-[#8ccaff]', color: 'text-[#235d84]', active: activeSettingsSection.value === 'security'},
  {label: 'Data & privacy', section: 'privacy', icon: ShieldCheckIcon, bg: 'bg-[#d7b7ff]', color: 'text-[#5b36a8]', active: activeSettingsSection.value === 'privacy'},
