...
export class SheepFileService {
    ...
    async upload(file: Express.Multer.File): Promise<SheepUploadedFile> {
        if (!this.apiKey) {
            throw new ServiceUnavailableException('Sheep file service API key is not configured');
        }
        ...
        try {
            ...
            throw new ServiceUnavailableException('Sheep file service is unavailable');
        }

        if (!response.ok) {
            const body = await response.text().catch(() => '');
            console.error('Sheep file service upload failed:', response.status, body);
            throw new InternalServerErrorException('Failed to upload file to sheep file service');
        }
        ...
        return {
            ...
            streamUrl: this.toAbsoluteUrl(uploaded.stream_url ?? this.streamUrl(uploaded.filename)),
        };
    }
    ...
    async fetchFile(filename: string, range?: string): Promise<Response> {
        ...
        if (!response.ok && response.status !== 206) {
            ...
            throw new InternalServerErrorException('Failed to fetch file from sheep file service');
        }
