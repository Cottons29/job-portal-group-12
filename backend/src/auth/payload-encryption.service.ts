...
export class PayloadEncryptionService {
  ...
  constructor() {
    const configuredPrivateKey = process.env.PAYLOAD_ENCRYPTION_PRIVATE_KEY?.replace(/\\n/g, '\n');
    const configuredPublicKey = process.env.PAYLOAD_ENCRYPTION_PUBLIC_KEY?.replace(/\\n/g, '\n');
